...
import com.season.hookplugin.compat.CompatibilityInfoCompat;
import com.season.hookplugin.compat.ProcessCompat;
...
import com.season.hookplugin.stub.ActivityStub;
import com.season.hookplugin.stub.ServiceStub;
                    ...
                    sHandle.post(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                LogPlugin.e(TAG, "preMakeApplication FAIL", e);
                            } finally {
                                mExec.set(true);
                                synchronized (lock) {
                                    lock.notifyAll();
            ...
            for (ActivityInfo info : infos) {
                if (TextUtils.equals(info.processName, myPname)) {
                    try {
                        ...
                        LogPlugin.e(TAG, "registerStaticReceiver error=%s", e, info.name);
                    }
                }
            }
        }
...
//        sSkipService.add("usagestats");
        ...
        try {
            ...
            if (SYSTEM_SERVICE_MAP == null) {
                try {
                    ...
                    LogPlugin.e(TAG, "readStaticField(SYSTEM_SERVICE_FETCHERS) from android.app.SystemServiceRegistry fail", e);
                }
            }
            ...
            if (SYSTEM_SERVICE_MAP != null && (SYSTEM_SERVICE_MAP instanceof Map)) {
                ...
                for (Object key : sSYSTEM_SERVICE_MAP.keySet()) {
                    ...
                    try {
                        ...
                        if (cause != null) {
                            LogPlugin.w(TAG, "Fake system service faile", e);
                        } else {
                            LogPlugin.w(TAG, "Fake system service faile", e);
                        }
                    } catch (Exception e) {
            ...
            LogPlugin.e(TAG, "fakeSystemServiceOldAPI", e);
        }
    }
    ...
    public static void fakeSystemService(Context hostContext, Context targetContext) {
        if (VERSION.SDK_INT >= VERSION_CODES.ICE_CREAM_SANDWICH_MR1 && !TextUtils.equals(hostContext.getPackageName(), targetContext.getPackageName())) {
            ...
            LogPlugin.i(TAG, "Fake SystemService for originContext=%s context=%s,cost %s ms", targetContext.getPackageName(), targetContext.getPackageName(), (System.currentTimeMillis() - b));
        }
    }
