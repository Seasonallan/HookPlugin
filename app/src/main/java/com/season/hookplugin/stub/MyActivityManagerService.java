...
import com.season.hookplugin.core.IPluginManagerImpl;
...
//        }
        ...
        try {
            ...
            LogPlugin.e(TAG, "error on read com.android.internal.R$styleable", e);
        }
    ...
    private void doGc(RunningAppProcessInfo myInfo) {
        ...
        if (activityCount <= 0 && serviceCount <= 0 && providerCount <= 0) {
            //杀死空进程。
            LogPlugin.i(TAG, "doGc kill process(pid=%s,uid=%s processName=%s)", myInfo.pid, myInfo.uid, myInfo.processName);
            try {
                android.os.Process.killProcess(myInfo.pid);
            } catch (Throwable e) {
                LogPlugin.e(TAG, "error on killProcess", e);
            }
            ...
            if (names != null && names.size() > 0) {
                for (String name : names) {
                    ...
                    LogPlugin.i(TAG, "doGc kill process(pid=%s,uid=%s processName=%s) service=%s", myInfo.pid, myInfo.uid, myInfo.processName, service);
                }
            }
        }
    }
