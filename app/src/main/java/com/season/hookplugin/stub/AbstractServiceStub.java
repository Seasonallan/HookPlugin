...
package com.season.hookplugin.stub;
...
public abstract class AbstractServiceStub extends Service {
            ...
            if (intent != null) {
                if (intent.getBooleanExtra("ActionKillSelf", false)) {
                    ...
                    if (!ServcesManager.getDefault().hasServiceRunning()) {
                        ...
                        LogPlugin.i(TAG, "doGc Kill Process(pid=%s,uid=%s has exit) for %s onStart=%s intent=%s", android.os.Process.myPid(), android.os.Process.myUid(), getClass().getSimpleName(), stopService, intent);
                    } else {
                        LogPlugin.i(TAG, "doGc Kill Process(pid=%s,uid=%s has exit) for %s onStart intent=%s skip,has service running", android.os.Process.myPid(), android.os.Process.myUid(), getClass().getSimpleName(), intent);
                    }
            ...
            try {
                new Thread() {
                    ...
                    public void run() {
                        ...
                        LogPlugin.i(TAG, "doGc Kill Process(pid=%s,uid=%s has exit) for %s 2", android.os.Process.myPid(), android.os.Process.myUid(), getClass().getSimpleName());
    ...
    private void handleException(Throwable e) {
        LogPlugin.e(TAG, "handleException", e);
    }
