...
class StaticProcessList {
    ...
    private Map<String, ProcessItem> items = new LinkedHashMap<String, ProcessItem>(10);
    ...
    void onCreate(Context mHostContext) throws NameNotFoundException {
        ...
        for (ResolveInfo activity : activities) {
            //LogTool.logV3("onCreate 添加预注册 " + activity.toString() + "  进程 " + activity.activityInfo.processName);
            //if (mHostContext.getPackageName().equals(activity.activityInfo.processName))
        ...
        if (packageInfo.providers != null && packageInfo.providers.length > 0) {
            for (ProviderInfo providerInfo : packageInfo.providers) {
                if (providerInfo.name != null && providerInfo.name.startsWith(ContentProvider.class.getName())) {
    ...
    private void addActivityInfo(ActivityInfo info) {
        ...
        //LogTool.logV2("addActivityInfo name = "+item.name);
